  ...
  return (
    <div className="max-w-[700px] mx-auto px-4 lg:px-6 py-8">
      {success && orderId && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center mb-6">
          ...
          <p className="text-gray-600 dark:text-zinc-300 text-sm mt-1">
            Your order ID is <span className="font-black text-black dark:text-white">{orderId}</span>. Save it to track your order.
          </p>
          <p className="text-xs text-gray-500 dark:text-zinc-400 mt-2">We will call you within 2 hours to confirm.</p>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border">
        ...
        <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">Enter your Order ID to see status</p>
      ...
      {searched && !order && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-8 border text-center mt-6">
          <div className="text-5xl">📦</div>
          <h3 className="font-bold mt-3">No order found</h3>
          <p className="text-sm text-gray-500 dark:text-zinc-400">Please check your Order ID and try again.</p>
        </div>
      )}

      {order && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border mt-6">
          <div className="flex justify-between items-start">
            <div>
              <div className="text-xs tracking-widest font-bold text-gray-500 dark:text-zinc-400">ORDER ID</div>
              <div className="font-black text-lg">{order.id || order.orderId}</div>
              <div className="text-xs text-gray-500 dark:text-zinc-400">Placed on {new Date(order.createdAt).toLocaleString()}</div>
            </div>
              ...
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={step.key} className="text-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto text-sm font-bold border-2 ${active ? "bg-black text-white border-black" : "bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-700 text-gray-400"}`}>{active ? "✓" : idx + 1}</div>
                    <div className={`text-xs font-bold mt-2 ${isCurrent ? "text-black dark:text-white" : active ? "text-black dark:text-white" : "text-gray-400"}`}>{step.label}</div>
                    <div className="text-[11px] text-gray-500 dark:text-zinc-400 hidden sm:block">{step.desc}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="border-t mt-8 pt-6">
            ...
            <div className="space-y-3 mt-3">
              {(order.items || []).map((it: any, i: number) => (
                <div key={i} className="flex gap-3">
                  <img src={it.image} alt="" className="w-14 h-14 rounded-lg object-cover bg-gray-50 dark:bg-zinc-800" />
                  <div className="flex-1">
                    <div className="font-medium text-sm line-clamp-1">{it.name}</div>
                    <div className="text-xs text-gray-500 dark:text-zinc-400">Size {it.size} • Qty {it.quantity}</div>
                  </div>
            ...
            <div className="border-t mt-4 pt-4 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-zinc-300">Payment</span>
            ...
            <div className="bg-gray-50 dark:bg-zinc-800 rounded-xl p-4 mt-6 text-sm">
              <div className="font-bold">Shipping Address</div>
              <div className="text-gray-600 dark:text-zinc-300 mt-1">
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="w-full border rounded-full py-2.5 text-sm font-bold hover:bg-gray-50 dark:bg-zinc-800"
              >
