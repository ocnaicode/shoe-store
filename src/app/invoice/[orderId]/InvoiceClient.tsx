...
export default function InvoiceClient({ orderId }: { orderId: string }) {
  ...
  if (!order) return <div className="text-center py-12"><h2 className="text-xl font-black">Order not found</h2><p className="text-sm text-gray-500">Check Order ID: {orderId}</p><Link href="/track-order" className="mt-4 inline-block bg-black text-white px-6 py-3 rounded-full font-bold">Track Order</Link></div>;
    ...
    <div className="max-w-3xl mx-auto px-4 lg:px-6 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Order #{order.id || order.orderId} • {new Date(order.createdAt).toLocaleDateString()}</p>
        </div>
      ...
      <div id="invoice" className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden">
        {/* Header */}
        <div className="bg-black text-white p-6">
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 bg-white text-black flex items-center justify-center font-black text-xl rounded">H</div>
                <div>
        ...
        <div className="p-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h3 className="font-black text-sm tracking-widest text-gray-500">BILL TO</h3>
              <div className="mt-2">
            ...
            <div className="md:text-right">
              <h3 className="font-black text-sm tracking-widest text-gray-500">INVOICE DETAILS</h3>
              <div className="mt-2 text-sm space-y-1">
                <div><span className="text-gray-500">Date:</span> <span className="font-bold">{new Date(order.createdAt).toLocaleDateString()}</span></div>
                <div><span className="text-gray-500">Payment:</span> <span className="font-bold uppercase">{order.paymentMethod}</span></div>
                <div><span className="text-gray-500">Shipping via:</span> <span className="font-bold">Steadfast Courier</span></div>
              </div>
            </div>
          </div>
                  ...
                  <tr key={i} className="border-b">
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        <img src={item.image} alt="" className="w-12 h-12 rounded-lg object-cover bg-gray-50 hidden sm:block" />
                        <div>
          ...
          <div className="mt-8 bg-gray-50 rounded-xl p-4 flex flex-col sm:flex-row justify-between gap-4 text-xs">
            <div>
              <div className="font-black">Thank you for shopping! 🙏</div>
              <div className="text-gray-600 mt-1">7 days easy return • 100% original products<br />Need help? Call 01700-000000 (10AM-10PM)</div>
            </div>
            <div className="text-right">
      ...
      <div className="mt-6 flex gap-3 justify-center">
        <Link href="/shop" className="border rounded-full px-6 py-3 font-bold hover:bg-gray-50">Continue Shopping</Link>
