  ...
  if (!user) {
    return (
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-16 text-center">
        <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto text-2xl">🔒</div>
        <h1 className="text-2xl font-black mt-4">Please Login</h1>
        <p className="text-sm text-gray-500 mt-2">Login to view your orders, invoices and profile. Checkout e auto account create hobe.</p>
        ...
        <div className="mt-8 bg-white rounded-2xl p-6 border max-w-md mx-auto text-left">
          <h3 className="font-black">Recent Orders (Guest)</h3>
          <p className="text-xs text-gray-500">Login korle shudhu apnar order dekhabe. Ekhon guest hisabe {orders.length} ta order ache.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-[#fbfbfb] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-8">
        ...
        <div className="grid lg:grid-cols-4 gap-6 mt-8">
          <div className="bg-white rounded-2xl p-4 border h-fit sticky top-[90px]">
          ...
          <div className="lg:col-span-3">
            {tab === "orders" && (
              <div className="space-y-4">
                <div className="bg-white rounded-2xl p-6 border flex items-center justify-between">
                    ...
                    <div key={o.id || o.orderId} className="bg-white rounded-2xl p-5 border">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="text-xs text-gray-500">{new Date(o.createdAt).toLocaleString()} • {o.paymentMethod?.toUpperCase()}</div>
                      ...
                      <div className="flex gap-2 mt-4 overflow-auto">
                        {(o.items || []).map((it: any, i: number) => (
                          <img key={i} src={it.image} alt="" className="w-16 h-16 rounded-xl object-cover bg-gray-50 border" />
                        ))}
                      </div>
                      <div className="flex items-center justify-between mt-4 border-t pt-4">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <Link href={`/track-order?orderId=${o.id || o.orderId}`} className="border px-4 py-2 rounded-full text-xs font-bold hover:bg-gray-50">
                            Track
                          </Link>
                          <a href={`/api/invoice?orderId=${o.id || o.orderId}`} target="_blank" className="border px-4 py-2 rounded-full text-xs font-bold hover:bg-gray-50 hidden sm:inline-flex">
                            Email
                          </a>
                        </div>
                      </div>
            ...
            {tab === "wishlist" && (
              <div className="bg-white rounded-2xl p-6 border">
                ...
                {wishlist.length === 0 ? (
                  <p className="text-sm text-gray-500 mt-4">No items in wishlist.</p>
                ) : (
            ...
            {tab === "addresses" && (
              <div className="bg-white rounded-2xl p-6 border">
                ...
                <div className="mt-4 border rounded-xl p-4">
                  ...
                  <div className="text-sm text-gray-600">House 12, Road 5, Gulshan-1, Dhaka - 1212</div>
                  <div className="text-xs text-gray-500">Phone: {user.phone || "01700000000"}</div>
              ...
              <div className="bg-white rounded-2xl p-6 border">
                ...
                <div className="grid sm:grid-cols-2 gap-4 mt-6">
                  ...
                  <label className="sm:col-span-2">
                    ...
                    <input value={profileForm.email} readOnly className="w-full mt-1 border rounded-xl px-4 py-3 text-sm bg-gray-50" />
